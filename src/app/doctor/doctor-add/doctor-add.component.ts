...
export class DoctorAddComponent {
  ...
  selectedFile!: File ;
constructor(public doctorService:DoctorService,private fb: FormBuilder,public router:Router){// form builder is a buitl in service u can use to amke ur life easier
  this.createForm();
}
createForm() {
  const strongPasswordPattern = /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?])(?=.*[^\da-zA-Z]).{8,}$/;
  ...
  this.form = this.fb.group({
    ...
    vezeeta: [0, [Validators.required, Validators.pattern(/^[0-9]+$/), Validators.min(100)]],
    password: ['',[Validators.required, Validators.minLength(6),Validators.pattern(strongPasswordPattern)]],
    email: ['',[Validators.required, Validators.email]],
    salary: [0,[ Validators.required,Validators.pattern(/^[0-9]+$/), Validators.min(3000)]],
    age: [0,[ Validators.required,Validators.pattern(/^[0-9]+$/), Validators.min(25)]],
